...
program
  .command('list-models')
  .description('List the OpenRouter model ids available for --model and config default-model.')
  .option('--json', 'output JSON on stdout')
  .action(async (opts) => {
    try {
      await runListModels({ json: !!opts.json });
    } catch (err) {
      exitWith(1, (err as Error).message);
    }
  });
...
program
  ...
  .option('-m, --model <id>', 'OpenRouter model id (e.g. google/gemini-3.1-pro-preview, anthropic/claude-opus-4.7). Default = config default-model > server default. List with `aigma list-models`.')
  .option('-n, --variants <count>', 'generate N takes (1-5) of the same prompt in parallel; one prompt + N HTML output nodes laid out vertically on the canvas', (v: string) => Math.max(1, Math.min(5, parseInt(v, 10) || 1)), 1)
